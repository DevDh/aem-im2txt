...
tf.flags.DEFINE_string("word_counts_output_file", HOME + "/im2txt/data/mscoco/word_counts.txt",
...
def _create_vocab(captions):
  ...
  #with tf.gfile.FastGFile(FLAGS.word_counts_output_file, "a+") as f:
  with open(FLAGS.word_counts_output_file, 'a+') as f:
...
def _process_metadata(captions, image_loc):
  ...
  num_captions = 0
  filename = image_loc
  processed_captions = _process_caption(captions)
  image_id = random.randint(111111,999999)
  image_metadata.append(ImageMetadata(image_id, filename, processed_captions))
  num_captions += len(captions)
  print("Finished processing %d captions for image at %s" % (num_captions, image_loc))

  return image_metadata


def prepare(image_loc, captions):
  ...
  train_dataset = _process_metadata(captions, image_loc)
